...
function ServiceVisual({ type }: { type: Service["visual"] }) {
  if (type === "film") {
    return (
      <div aria-hidden="true" className="relative h-[7.2rem] w-[9.6rem] shrink-0 overflow-hidden rounded-[1.15rem] bg-[linear-gradient(145deg,#121722_0%,#2d3950_58%,#d9e3ff_100%)] shadow-inner">
        <div className="absolute left-4 top-4 h-4 w-10 rounded-md bg-white/15" />
        <div className="absolute bottom-5 left-5 h-12 w-20 rounded-lg bg-[#0b0f17] shadow-xl">
          <div className="absolute -right-4 top-3 h-7 w-7 rounded-full border-[5px] border-[#316bff] bg-white" />
          <div className="absolute left-3 top-3 h-3 w-8 rounded bg-white/30" />
        </div>
        <div className="absolute bottom-3 right-3 h-9 w-9 rounded-full bg-[#316bff]/90 blur-[0.5px]" />
      </div>
    );
  }
  ...
  if (type === "learning") {
    return (
      <div aria-hidden="true" className="relative h-[7.2rem] w-[9.6rem] shrink-0 overflow-hidden rounded-[1.15rem] bg-[linear-gradient(145deg,#e9f0ff_0%,#c9d8ff_60%,#ffffff_100%)] shadow-inner">
        <div className="absolute inset-x-3 top-4 h-[4.7rem] rounded-xl border-2 border-[#316bff]/25 bg-white shadow-md">
          <div className="absolute left-3 top-3 h-7 w-8 rounded-md bg-[#316bff]/10" />
          <div className="absolute right-3 top-3 h-7 w-12 rounded-md bg-[#316bff]/15" />
          <div className="absolute left-1/2 top-1/2 flex h-9 w-9 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-[#316bff] text-sm text-white">▶</div>
        </div>
      </div>
    );
  }
  ...
  if (type === "awareness") {
    return (
      <div aria-hidden="true" className="relative h-[7.2rem] w-[9.6rem] shrink-0 overflow-hidden rounded-[1.15rem] bg-[linear-gradient(145deg,#eef3ff_0%,#d8e2ff_55%,#ffffff_100%)] shadow-inner">
        <div className="absolute left-5 top-7 h-10 w-12 -rotate-6 rounded-l-full bg-[#316bff]" />
        <div className="absolute left-[3.6rem] top-[2.35rem] h-8 w-11 -rotate-6 bg-[#316bff] [clip-path:polygon(0_18%,100%_0,100%_100%,0_82%)]" />
        <div className="absolute left-8 top-[4.65rem] h-8 w-3 rotate-[-18deg] rounded-full bg-[#0b0f17]" />
        <span className="absolute right-5 top-6 h-[3px] w-7 rotate-[-18deg] rounded bg-[#316bff]" />
        <span className="absolute right-3 top-12 h-[3px] w-8 rounded bg-[#316bff]" />
        <span className="absolute right-5 top-[4.3rem] h-[3px] w-7 rotate-[18deg] rounded bg-[#316bff]" />
      </div>
    );
  }

  return (
    <div aria-hidden="true" className="relative h-[7.2rem] w-[9.6rem] shrink-0 overflow-hidden rounded-[1.15rem] bg-[linear-gradient(145deg,#161a2e_0%,#393a7a_48%,#8a6dff_100%)] shadow-inner">
      <div className="absolute inset-x-4 bottom-4 h-[4.4rem] rounded-lg border border-white/25 bg-[#0b0f17]/75 shadow-xl">
        <div className="absolute left-3 top-3 h-7 w-7 rounded-full bg-[#316bff]" />
        <div className="absolute left-12 top-3 h-2 w-10 rounded bg-white/25" />
        <div className="absolute left-12 top-7 h-2 w-8 rounded bg-white/15" />
      </div>
      <div className="absolute right-3 top-3 rounded-lg bg-[#316bff] px-2.5 py-2 text-xs font-bold text-white">AI</div>
    </div>
  );
}
...
function QuillDirector({ reduceMotion, opacity, scale }: {
  ...
  return (
    <motion.figure
      ...
      className="relative z-20 mx-auto mb-10 w-full max-w-[20rem] lg:absolute lg:left-1/2 lg:top-[0.5%] lg:mb-0 lg:w-[30%] lg:max-w-[23rem] lg:-translate-x-1/2"
    >
      <div className="relative mx-auto h-[31rem] sm:h-[34rem] lg:h-[34rem]">
        <img
      ...
      <figcaption className="relative z-30 -mt-1 text-center">
    ...
    >
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr] lg:items-center">
        <div className="flex items-center gap-5">
          <div aria-hidden="true" className="hidden h-20 w-32 shrink-0 overflow-hidden rounded-xl bg-[linear-gradient(145deg,#0b0f17,#316bff)] p-3 sm:block">
            <div className="h-full rounded-lg bg-white/10 p-2">
              <div className="h-2 w-14 rounded bg-white/70" />
              <div className="mt-2 h-2 w-20 rounded bg-white/30" />
              <div className="mt-3 flex gap-2"><span className="h-6 w-6 rounded-full bg-white/70" /><span className="h-6 w-6 rounded-full bg-white/35" /><span className="h-6 w-6 rounded-full bg-white/20" /></div>
            </div>
          </div>
          <div>
...
export default function QuillServices() {
  ...
  const quillOpacity = useTransform(scrollYProgress, [0, 0.04], [0.55, 1]);
  const quillScale = useTransform(scrollYProgress, [0, 0.08], [0.98, 1]);
            ...
            <div className="lg:hidden">
              {services.map((service) => (
                <motion.article key={service.number} initial={reduceMotion ? false : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} className="relative z-20 mb-5 overflow-hidden rounded-[1.6rem] border border-black/10 bg-white p-6 shadow-[0_18px_60px_rgba(11,15,23,0.08)]">
                  <div className="flex items-start justify-between gap-4"><div><p className="sld-label text-[#316bff]">{service.number}</p><h3 className="mt-3 text-2xl font-medium tracking-[-0.04em]">{service.title}</h3></div><ServiceVisual type={service.visual} /></div>
